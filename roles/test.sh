...
then
    ...
    docker run --detach --volume="$role_dir":/etc/ansible/roles/role_under_test:rw \
        --name $container_id $opts geerlingguy/docker-$distro-ansible:latest $init
fi

# Prepare container if `before.yml` is present.
if [ -f "$role_dir/tests/before.yml" ]; then
  printNormal "Prepare container; Running preparation."
  ${DOCKER} ansible-playbook /etc/ansible/roles/role_under_test/tests/before.yml
fi
...
${DOCKER} ansible-playbook /etc/ansible/roles/role_under_test/tests/$playbook --syntax-check

# Run Ansible playbook.
printNormal "Running command: ${DOCKER} ansible-playbook /etc/ansible/roles/role_under_test/tests/$playbook"
${DOCKER} ansible-playbook /etc/ansible/roles/role_under_test/tests/$playbook

# Idempotence test
if [ "$test_idempotence" = true ]; then
  printNormal "Running playbook again: idempotence test"
  idempotence=$(mktemp)
  ${DOCKER} ansible-playbook /etc/ansible/roles/role_under_test/tests/$playbook | tee -a $idempotence
